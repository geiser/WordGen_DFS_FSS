...
```{r, include=FALSE}
dat <- readxl::read_excel("Learn_Vocabulario.xlsx", sheet = "data")
...
```{r, include=FALSE}
(df <- get.descriptives(dat, "pos.score", c("grupo"), include.global = F, symmetry.test = T))
```
...
```{r, include=FALSE}
(df <- get.descriptives(dat, "pos.score", c("grupo","genero","zona"), include.global = F, normality.test = T))
```
...
p.tt <- paired.ttest(pair.df.experimental, "score","time", "greater", as.list = T)
(pdf <- p.tt$t.test)
```
...
*Note*: There is significant difference in the experimenatal group (apenas a intervenção causa efeito na aprendizagem)
